...
Mojolicious::Plugin::OpenAPI::Guides::OpenAPI3 - Mojolicious <3 Open API (Swagger)
...
Here OpenAPIv3 input differs from v2 spec, where C<paramters> is used for input in the
...
The securityScheme is added under components, where on way is to have the client
...
Its then referenced under the path object as security like this
...
You can then utilize security, by adding security callback when loading
the plugin
  ...
  $self->plugin(
    OpenAPI => {
      ...
      schema   => 'v3',
      security => {
        apiKey => sub ($c, $definition, $scopes, $cb) {
          if (my $key = $c->tx->req->content->headers->to_hash->{'X-Api-Key'}) {
...
Online and offline tools are available one example is
...
=head2 Doc rendering of OpenAPI v3 spec

The internal doc renderer does not support OpenAPI v3, and issue exists for this
on github L<jhthorsen/mojolicious-plugin-openapi#127>
